<div class="section section__how-it-works">
    <h2>How it works</h2>
    <div class="content">
        <div class="left">
            <img src="<?php echo get_theme_file_uri("images/soil-gel/polymerplate.png") ?>" alt="image of polymers on a plate">
            <p>Soil-Gel is a Potassium based </br> Super Absorbent Polymer for Agriculture.</p>
        </div>
        <div class="right">
            <img src="<?php echo get_theme_file_uri("images/soil-gel/polymerroot.png") ?>" alt="image of polymer mixed into soil">
            <p>It acts as a water-retaining agent for plants by </br> absorbing, storing and releasing water at the roots when needed.</p>
        </div>
    </div>
</div>